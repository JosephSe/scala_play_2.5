...
trait ATGDao {
  ...
  override def executeQuery(entity: Option[String]): System =  entity match {
    ...
    override def executeAtgQueryForPropertyContractStatus(entity: Option[String]) : List[Property] =  {
      db.withConnection(conn => {
        entity.get match {
          case "fit_contract" => val rs = (conn createStatement).executeQuery(getAtgDetailsQuery(entity.get))
    ...
    override def executeAtgQueryForPropertyContractCurrency(entity: Option[String]) : List[Property] =  {
      db.withConnection(conn => {
        entity.get match {
          case "fit_contract" => val rs = (conn createStatement).executeQuery(getAtgCntByCurrencyQuery(entity.get))
    ...
    override def executeAtgQueryForPropertyContractModel(entity: Option[String]) : List[Property] =  {
      db.withConnection(conn => {
        entity.get match {
          case "fit_contract" => val rs = (conn createStatement).executeQuery(getAtgCntByModelQuery(entity.get))
